...
void setup() {
  ...
  digitalWrite(TFT_CS3, HIGH); // Touch controller chip select (if used)
  digitalWrite(TFT_CS2, HIGH); // TFT screen chip select
  digitalWrite(TFT_CS4, HIGH); // SD card chips select, must use GPIO 5 (ESP32 SS)

  tft.begin();

  if (!SD.begin(TFT_CS4, tft.getSPIinstance())) {
